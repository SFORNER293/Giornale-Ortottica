...
export const SpotDifferencesGame: React.FC<SpotDifferencesGameProps> = ({ data, onComplete, isCompleted }) => {
  ...
  const renderSceneContent = (isSideB: boolean) => {
    const scene = data.sceneType || 'studio_fissazione';
    ...
    if (scene === 'studio_strabismo') {
      return (
        <>
          ...
          <circle cx={isSideB ? "281" : "285"} cy="38" r="3" fill="black" /> {/* Esotropia in B */}
    ...
    if (scene === 'cabinet_sinottoforo') {
      return (
        <>
          ...
          {isSideB ? null : <circle cx="185" cy="63" r="2.5" fill="white" />} {/* Coda */}
        </>
      );
    }
